...
tx.executeSql('CREATE TABLE IF NOT EXISTS mydata (id integer primary key, usuario text, contra text)');
});

//<!--Method to insert new row in the database-->
$(document).on('click', '#creat', function(){
var name = $("#usuario").val();
var email = $("#contraseña").val();
db.transaction(function(transaction) {
var executeQuery = "INSERT INTO mydata (usuario, contra) VALUES (?,?)";
...
var pair="<tr><th data-priority=\"1\"><center>Id</center></th><th data-priority=\"1\"><center>usuario</center></th><th data-priority=\"2\"><center>contraseña</center></th><th><center>actualizar</center></th><th><center>eliminar</center></th></tr>";
var i=0;
...
db.transaction(function(transaction) {
var executeQuery = "";
transaction.executeSql("UPDATE mydata SET usuario=?, contra=? WHERE id=?", [name,email,id],
...
$("#id").val(id);
db.transaction(function(transaction) {
transaction.executeSql('SELECT usuario,contra FROM mydata where id=?', [id], function (tx, results) {
